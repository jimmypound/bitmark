...
static bool ParseArgs(ArgsManager& args, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitmarkd -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bitmarkd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for bitmarkd but not for the GUI so do this here

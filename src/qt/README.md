This directory contains the source code for the Bitmark Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bitmark UI.
...
- ClientModel is used by the main application `bitmarkgui` and several models like `peertablemodel`.
...
4. Enter `bitmark-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitmark-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bitmark-qt`
9. You're all set! Start developing, building, and debugging the Bitmark Core GUI

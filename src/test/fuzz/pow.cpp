...
FUZZ_TARGET(pow, .init = initialize_pow)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes() > 0, 10000) {
        ...
        }
        {
            ...
            (void)CalculateNextWorkRequired(&current_block, fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(0, std::numeric_limits<int64_t>::max()));
            if (current_block.nHeight != std::numeric_limits<int>::max() && current_block.nHeight - (consensus_params.DifficultyAdjustmentInterval() - 1) >= 0) {
                (void)GetNextWorkRequired(&current_block, &(*block_header), Algo::SHA256D);
            }
        }
        {
        ...
        }
        {
            ...
            if (hash) {
                (void)CheckProofOfWork(*hash, fuzzed_data_provider.ConsumeIntegral<unsigned int>(), Algo::SHA256D);
            }
        }
    }
}
...
FUZZ_TARGET(pow_transition, .init = initialize_pow)
{
    ...
    unsigned int new_nbits{GetNextWorkRequired(last_block, nullptr, Algo::SHA256D)};
    Assert(PermittedDifficultyTransition(last_block->nHeight + 1, last_block->nBits, new_nbits));
}

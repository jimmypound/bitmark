...
static void TestBlockSubsidyHalvings(const Consensus::Params& consensusParams)
{
    ...
    for (int nHalvings = 0; nHalvings < maxHalvings; nHalvings++) {
        ...
        CAmount nSubsidy = GetBlockSubsidy(NULL, nHeight);
        BOOST_CHECK(nSubsidy <= nInitialSubsidy);
        BOOST_CHECK_EQUAL(nSubsidy * 2, nPreviousSubsidy);
        nPreviousSubsidy = nSubsidy;
    }
    BOOST_CHECK_EQUAL(GetBlockSubsidy(NULL, maxHalvings * consensusParams.nSubsidyHalvingInterval), 0);
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    BOOST_CHECK_EQUAL(nSum, CAmount{2757987367596000});
}
